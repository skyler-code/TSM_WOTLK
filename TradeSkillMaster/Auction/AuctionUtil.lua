...
do
	local fPrivate = {modes={}} function fPrivate:GetModeObject(obj, buttonText, buttonDesc, module) local temp, mt, mt2, test = {}, {}, {} mt = { data = {obj=obj, buttonText=buttonText, buttonDesc=buttonDesc, module=module}, __eq = function(a, b) test = time() return not (a[b] and b == "isValid" and mt.data.isValid) end, __index = function(f, i) return mt.data[i] end, __newindex = function(...) local num = select('#', ...) local _, i, v = ... if i == "result" then if num == 2 then mt.isValid = true elseif v == "invalid" then mt.isValid = false elseif type(v) == "string" and strfind(v, "AuctionGUI") then mt.isValid = true else wipe(fPrivate) error("Attempt to update a read-only table.", 2) end else wipe(fPrivate) error("Attempt to update a read-only table.", 2) end end, } mt2 = { __newindex = function(_, i, v) if i == "isValid" then mt.data[i] = v else wipe(fPrivate) error("Attempt to update a read-only table.", 2) end end, } setmetatable(mt, mt2) setmetatable(temp, mt) return temp end function fPrivate:GetAHTabFrame() local temp = CreateFrame("Frame", nil, TSMAuctionFrame, "AuctionFrameTabTemplate") local frame = {} local mt = { __index = function(f, i) if i == "modeText" then return temp.modeText elseif i == "AddSecureChild" then return function(_, child) return temp:AddSecureChild(f, child) end elseif i == "Validate" then return function() return temp end elseif type(temp[i]) == "function" then return function(_, ...) temp[i](temp, ...) end elseif type(temp[i]) == "table" then return temp[i] else error("Invalid table read.", 2) end end, __newindex = function(_, i, v) if i == "mode" then temp[v[1]] = v[3][v[2]] elseif i == "content" or i == "controlFrame" then temp[i] = v else wipe(fPrivate) error("Attempt to update a read-only table.", 2) end end } setmetatable(frame, mt) return frame end local function isDoubleEqual(x, y) return abs(x-y) < 0.01 end function fPrivate:CheckEnv() local mode = fPrivate.mode if mode.previous then local current = {GetCursorPosition()} if isDoubleEqual(current[1], mode.previous[1]) and isDoubleEqual(current[2], mode.previous[2]) and (GetTime() - mode.previous[3]) > 120 then mode.test = true return true end end mode.previous = {GetCursorPosition()} mode.previous[3] = GetTime() end function fPrivate:OnSidebarButtonClick(mode, button) assert(mode.obj, "Invalid mode: "..(fPrivate.failed and 1 or 0)) fPrivate:HideCurrentMode() fPrivate.frame.mode = {"modeText", "buttonText", mode} fPrivate.mode = mode.obj fPrivate.mode:Show(fPrivate.frame, button) return mode.info end function TSM:GetAuctionFramePrivate() TSM.GetAuctionFramePrivate = nil if tonumber(select(3, strfind(debugstack(), "([0-9]+)"))) == 113 then fPrivate.num = 432 return fPrivate end end
end